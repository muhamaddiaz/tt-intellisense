...
exports.preprocess = exports.parse = void 0;
...
const parse = (text) => {
    ...
    while ((match = root.content.slice(i).match(regex_1.default)) !== null) {
        ...
        const scriptBlock = match.groups.scriptBlock;
        if (!matchText &&
            !expression &&
            !statement &&
            !ignoreBlock &&
        ...
        if (scriptBlock != undefined) {
            const openingTag = match.groups.scriptOpeningTag;
            const closingTag = match.groups.scriptClosingTag;
        ...
        if (statement != undefined) {
            ...
            if (keyword.startsWith("END")) {
                ...
                while (!start) {
                    ...
                    if (!start) {
                        throw new Error(`No closing statement found for "[% ${last_start} %]".`);
                    }
                    last_start = start.content;
                    if (start.keyword == "ELSE" ||
                        start.keyword == "ELSIF" ||
                        start.keyword == "CASE" ||
                        start.keyword == "CATCH" ||
                        start.keyword == "FINAL") {
...
const placeholderGenerator = (text) => {
    ...
    return () => {
        while (true) {
            id++;
            const placeholder = tt_1.Placeholder.startToken + id + tt_1.Placeholder.endToken;
