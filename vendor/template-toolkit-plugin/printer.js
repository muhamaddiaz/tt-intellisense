...
const getVisitorKeys = (ast) => {
    ...
    return Object.values(ast)
        .filter((node) => {
        return node.type === "block" || node.type === "script";
    })
        .map((e) => e.id);
};
...
const printStatement = (node) => {
    ...
    if (["ELSE", "ELSIF", "CASE", "CATCH", "FINAL"].includes(node.keyword) &&
...
const printScriptBlock = (node) => {
    // For empty scripts or those with only attributes, don't add extra newlines
    if (!node.content.trim()) {
        return doc_1.builders.group([node.openingTag, node.closingTag]);
    }
    return doc_1.builders.group([
        node.openingTag,
        doc_1.builders.indent(getMultilineGroup(node.content.trim())),
        doc_1.builders.hardline,
        node.closingTag,
    ]);
};
const hasTemplateTags = (content) => {
    return content.includes("[%") || content.includes("%]");
};
const embed = () => {
    return async (textToDoc, print, path, options) => {
        const node = path.getNode();
        if (!node || !["root", "block", "script"].includes(node.type)) {
            return undefined;
        }
        if (node.type === "script") {
            // Don't try to format if it contains template tags
            if (hasTemplateTags(node.content)) {
                return undefined;
            }
            try {
                // Format JavaScript content
                const formattedJs = await textToDoc(node.content.trim(), {
                    ...options,
                    parser: "babel",
                });
                // For empty scripts or those with only attributes, don't add extra newlines
                ...
                return doc_1.builders.group([
                    node.openingTag,
                    doc_1.builders.indent([doc_1.builders.hardline, formattedJs]),
                    doc_1.builders.hardline,
                    node.closingTag,
                ]);
            }
            catch (error) {
                // If JavaScript formatting fails, fall back to print method
                return undefined;
            }
        }
...
const splitAtElse = (node) => {
    const elseNodes = Object.values(node.nodes).filter((n) => n.type === "statement" &&
        ["ELSE", "ELSIF", "CASE", "CATCH", "FINAL"].includes(n.keyword) &&
...
const findPlaceholders = (text) => {
    ...
    let i = 0;
    while (true) {
        const start = text.slice(i).search(tt_1.Placeholder.startToken);
        if (start === NOT_FOUND)
            break;
        const end = text
            .slice(start + i + tt_1.Placeholder.startToken.length)
            .search(tt_1.Placeholder.endToken);
        if (end === NOT_FOUND)
            break;
        res.push([
            start + i,
            end + start + i + tt_1.Placeholder.startToken.length + 1,
        ]);
        i += start + tt_1.Placeholder.startToken.length;
    }
    return res;
};
